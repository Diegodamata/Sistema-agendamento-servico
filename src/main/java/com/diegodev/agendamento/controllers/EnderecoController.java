...
public class EnderecoController implements GenericController{
    ...
    @PostMapping
    public ResponseEntity<Void> adicionarEndereco(@RequestBody EnderecoRequestDTO requestDTO){
        Endereco enderecoAdicionado = enderecoService.adicionarEndereco(requestDTO.toEndereco());

        URI uri = gerarHeaderLocation(enderecoAdicionado.getId());
